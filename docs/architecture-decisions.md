...
## 33. 분류는 AI 자동 + 사용자 정정(category·tags), 카테고리 모델 확장은 보류

분류에는 축이 둘이다 — **category**(고정 enum 3개, 상단 탭 필터용)와 **tags**(AI가 자유 생성, `tags` 테이블). "열린 자유 분류"의 절반은 이미 태그가 한다. category는 일부러 좁게 고정한 것이다(#20에서 JOB→CAREER 통합). 여기서 두 질문이 나왔다 — ① AI 자동 분류를 사용자가 고칠 수 있어야 하지 않나(오분류 정정), ② 페르소나를 IT 너머로 넓히면 category를 사용자가 만들거나 AI가 자동 생성해야 하지 않나.

**①은 한다(이번 작업).** AI 오분류는 필연이고, 고칠 길이 없으면 잘못 박힌 아이템이 기대한 탭/태그에서 사라져 신뢰가 깨진다. 이미 #31에서 "AI·재크롤이 사용자가 손댄 걸 덮어쓰면 안 된다"(`titleEditedByUser`)는 원칙을 세웠고, category·tags에도 그대로 적용한다.
- **카테고리 수정**: `PATCH /items/{id}`에 `category` 추가(title·memo 받던 그 엔드포인트). + `categoryEditedByUser` 플래그 — 안 두면 `reclassify-all`(`findForReclassify`가 null·ETC를 잡음)이 사용자가 ETC로 고친 걸 다시 분류하거나 재분류 때 태그를 비운다. 그래서 재분류 대상 쿼리에서 `categoryEditedByUser=true`를 제외한다(#31 가드의 카테고리판).
- **태그 추가/삭제**: 아이템 단위로 join 행만 더하고 뺀다(`POST`/`DELETE /items/{id}/tags`). 공유 Tag 자체는 유지(삭제 시 다른 아이템 영향 없음). 태그 **이름 변경(rename)**은 공유 Tag라 전역 영향이라 범위에서 제외(별도 과제).
- get-or-create의 race-safe 로직(#28: `INSERT IGNORE` + 잠금 읽기)이 이제 자동 분류와 수동 추가 두 곳에서 필요해, `ItemClassifier`에 사적으로 있던 걸 `TagService.getOrCreate`로 빼 공유한다(중복 구현 방지).

**②는 보류.** 끌리는 답은 "AI가 카테고리를 자유 생성"이지만 그게 제일 위험하다 — "개발/프로그래밍/코딩"처럼 같은 뜻이 난립해 상단 탭이 불안정해지고, 그 "열린 자유 분류"는 **이미 태그가 하는 일**이라 두 축이 겹친다. enum을 끝없이 늘리는 것도 누구의 삶도 다 못 담아 ETC 과포화 + 매번 배포·마이그레이션이다. 방향을 정해 못박아둔다: *고정 분류는 도메인이 좁을 때만 작동하고, 도메인이 열리면 전역 자동 taxonomy가 아니라 **사용자 소유 구조 + AI 제안**이 답이다* — 사용자가 자기 카테고리/컬렉션을 만들고(또는 태그 위에 이름붙인 저장된 뷰), AI는 "여기 넣으면 어때요?"를 제안만 한다. 다만 지금 페르소나(IT 취준/주니어)엔 3-enum이 충분하고, `@Enumerated(STRING)` enum → 임의 카테고리는 `categories` 테이블 신설 + 마이그레이션이라 무게가 크다. 페르소나 확장이 *실제로 결정될 때* 착수한다. 그 전까지 enum 유지.

---

## 34. 재방문 마찰은 네이티브 앱이 아니라 refresh 수명 연장 + PWA로 푼다

"매번 크롬 열고 로그인하는 게 번거롭다 → 모바일 앱으로 만들어 자동 로그인하면 들어가기만 하면 되지 않나"에서 출발했다. 코드를 보고 마찰의 정체를 먼저 규명했다 — access 30분 / **refresh 14일 고정**(`JWT_REFRESH_EXPIRATION=1209600000`)이고, #11에서 회전(rotation)을 제거해 `/reissue`는 access만 새로 주고 refresh는 *최초 로그인 시점부터 14일*에서 안 늘어난다(슬라이딩 없음). 즉 활발히 써도 최대 14일마다 강제 재로그인이고, 동선은 크롬 주소창 직접 진입이다. 마찰의 원인은 "웹이라서"가 아니라 이 둘이다.

선택지는 (A) refresh 수명 연장 + PWA 홈화면 설치, (B) Capacitor로 기존 React를 네이티브 래핑, (C) React Native/Flutter 재작성. **(A)를 택했다.** 네이티브의 "자동 로그인"은 마법이 아니라 *기기에 오래 사는 자격증명을 저장*하는 것이고, 그건 웹/PWA도 긴 수명 refresh 쿠키로 동일하게 한다 — 네이티브는 같은 효과를 훨씬 비싸게 다시 만들면서, #11~#13에서 우아하게 푼 `httpOnly 쿠키 + Vercel 동일출처 프록시` 인증을 토큰+secure storage 방식으로 재설계해 그 보안 숙제를 되연다. 제품 명제(유효기간 없는 영속성 + 분야별 정리)는 이미 웹이 제공하고, 앱이 바꾸는 건 *접근 편의*뿐이라 그 레이어(인증/PWA)에서 푼다.

- **refresh 수명 연장**: 14일 → 60~90일. 개인 아카이브 + httpOnly·Secure·SameSite=Lax·Path 한정이라 탈취 위험 증가를 수용 가능. 슬라이딩 재도입(매 reissue마다 refresh 재발급)은 #11의 회전 레이스를 되살리므로 안 하고, 단순 장기 만료를 택한다(무한은 보안상 지양 — 강제 로그아웃 주기를 길게 늘릴 뿐).
- **PWA 홈화면 설치**: manifest·아이콘은 이미 있음. 크롬 주소창 대신 아이콘 탭 → 풀스크린 아카이브. 코드 변경 최소(필요 시 서비스워커만 보강).

네이티브가 정당해지는 단 하나의 조건: 위를 했는데도 **iOS standalone PWA가 세션/쿠키를 주기적으로 비우면**(알려진 iOS 동작) Capacitor 래퍼 + Keychain 토큰 저장이 iOS 한정 처방으로 정당해진다. 단 *실측으로 확인한 뒤*다 — 가정 위에 네이티브를 짓지 않는다. (동기가 포트폴리오/학습이면 네이티브는 별개로 정당한 선택.)

- 아쉬운 점 / 한계: 장기 refresh도 영구는 아니라 분기마다 한 번은 재로그인. iOS PWA 세션 유지 신뢰성은 아직 미검증 가정이라, (A) 적용 후 며칠 사용으로 확인이 필요하다.

---

## 35. 웹 캡처를 링크/텍스트/이미지로 완성하고, TEXT 본문을 보이게(+편집) 한다

봇은 링크·텍스트·이미지 셋 다 받는데(설계결정 #30), 웹 FAB는 **링크만** 추가할 수 있었다. 그래서 ① 웹에서 메모/사진을 남기려면 카톡으로 가야 했고(#31·#32에서 "웹 이미지 경로 없음"으로 미뤄둔 그 빚), ② 봇으로 보낸 TEXT는 탭해도 본문이 안 보였다 — `processText`가 title=앞 100자/content=전체로 저장하는데 `ItemDetailModal`이 `content`를 아예 그리지 않아 "내용이 전부 제목에 들어간" 것처럼 느껴졌다. 사실은 *본문을 보여줄 자리가 없던 것*이다.

**(1) 웹 멀티 추가** — FAB를 타입 선택 시트(링크/텍스트/이미지)로 바꾼다. 봇과 저장 로직을 공유하고 웹은 동기 처리(설계결정 #17과 동일 원칙).
- 링크: 기존 `addLink` 그대로.
- 텍스트: 봇 `processText`의 저장 로직을 공유(`saveText`)해 TEXT 아이템 생성 + 분류. 웹은 동기 반환.
- 이미지: `S3Service.uploadImage`(관리자 팝업용으로 이미 있던 magic-byte 검증 업로드)를 사용자 이미지에 재사용 + **#32 `classifyImage`로 캡션·카테고리·태그 자동**. 이로써 #31/#32가 남긴 "웹 이미지 경로 없음"이 닫히고, 업로드 이미지도 봇 사진과 동일하게 AI 캡션이 붙는다. multipart 업로드라 파일 크기 상한을 둔다.

**(2) TEXT 본문 표시·편집** — 봇/웹 공통.
- 저장: 제목 = **앞 100자 → 첫 줄**(짧은 라벨), content = 전체. 100자 중간에서 끊기는 것보다 첫 줄이 라벨로 자연스럽다.
- 표시: `ItemDetailModal`에 본문(content)을 보이게 하고, **TEXT 타입은 본문 편집까지** 연다. 카드엔 본문 미리보기 몇 줄.
- **content 편집은 TEXT 타입에만** 허용한다. LINK·IMAGE의 content는 OG/AI에서 파생된 값이라 사용자가 고쳐도 재크롤·재분류가 덮어쓰고 의미가 모호하다. TEXT의 content는 사용자가 직접 쓴 글이라 1급 편집 대상이다. `PATCH /items/{id}`에 content를 받되 타입 가드로 분기한다.

**(3) memo는 그대로 분리 유지.** 독립 TEXT 아이템(그 자체가 내용)과 기존 아이템에 다는 한 줄 주석(memo, #31)은 의도가 다르다. 합치면 둘 다 모호해진다.

- 아쉬운 점 / 한계: 이미지 동기 업로드는 요청 시간을 늘린다(비전 호출 포함) — 봇 비동기와 달리 사용자가 기다린다. 파일 크기 상한·로딩 표시로 완화. 과거 봇 TEXT 행은 title=앞 100자 그대로지만(데이터 마이그레이션 안 함) 본문 표시가 생겨 더는 안 가려진다. content 편집을 TEXT로만 막는 건 타입 분기라, 추후 IMAGE 캡션 직접 편집 같은 요구가 오면 별도 설계가 필요하다.
